...
export default function PapersBrowsePage() {
  ...
  const submitMpesaPayment = async () => {
    ...
    try {
      ...
      setPaymentStatus({
        status: 'success',
        message: '✅ STK Push sent! Check your phone and enter your M-Pesa PIN to complete payment. Access will be granted automatically once payment is confirmed.',
      });

      // Close modal after a delay — real access is granted via M-Pesa callback
      setTimeout(() => {
        setShowPaymentModal(null);
        setMessage({ text: 'Payment initiated. Check your phone to complete. Access will be granted automatically.', isError: false });
      }, 4000);
    } catch (error: any) {
        ...
        <div className="fixed inset-0 z-50 md:hidden">
          ...
          <aside className="absolute left-0 top-0 bottom-0 w-72 bg-white shadow-2xl flex flex-col">
            <div className="p-6 border-b border-slate-100">
              <div className="flex items-center justify-between">
                <Link href="/dashboard" className="flex items-center gap-3">
                  ...
                  <span className="text-xl font-bold text-slate-900">StudyPal</span>
                </Link>
                <button onClick={() => setSidebarOpen(false)} className="p-2 rounded-lg text-slate-500 hover:bg-slate-100">✕</button>
              </div>
            </div>
            <nav className="flex-1 p-4 space-y-1">
              {[
                ...
                <Link key={link.href} href={link.href} onClick={() => setSidebarOpen(false)}
                  className={`flex items-center gap-3 px-4 py-3 rounded-xl font-semibold text-sm transition-all ${link.active ? 'bg-indigo-600 text-white' : 'text-slate-500 hover:bg-slate-100 hover:text-slate-900'}`}>
            ...
            <div className="p-4 border-t border-slate-100">
              <button onClick={() => { localStorage.removeItem('token'); localStorage.removeItem('user'); router.push('/auth/login'); }}
